...
async function init() {
  ...
  const apiResponse = await chrome.runtime.sendMessage({ type: 'GET_API_URL' });
  const baseUrl = apiResponse?.url || 'https://app.formstatus.co';
  if (forgotPasswordLink) forgotPasswordLink.href = `${baseUrl}/forgot-password`;
  if (registerLink) registerLink.href = `${baseUrl}/register`;
